...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        for episode in range(0, max_episodes+1):
            ...
            state[:,:,0] = state_stock[2] #state[:,:,0] = state_stock[0]; state에 대한 RGB처럼 두개의 구성셋으로 구성함
            #print("state outside")
            #print(state)
            ...
            while not end == True: #이부분이 한 episode에서 state를 진행시키는 부분 actor critic을 이 부분에 넣어야함.
                ...
                next_state[:,:,0] = next_state_stock[2] #next_state[:,:,0] = next_state_stock[0]; next_state에 대한 RGB처럼 두개의 Channel  
