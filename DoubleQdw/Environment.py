...
u_t_avg = np.zeros([nyp], dtype=np.float64); rms_t_avg = np.zeros([nyp, 3], dtype=np.float64)
...
u_t_avg_gpu = cp.asarray(u_t_avg); rms_t_avg_gpu = cp.asarray(rms_t_avg)
...
class env(): 
    ...
    def reset(self, episode): #초기화하고 state를 보내준다
        ...
        if episode == 0:
            data_frame_init = pd.read_csv(r"/home/fish991/code/Half_staggered/Env_wide/developed2.csv", encoding = "ISO-8859-1")
        elif episode ==1:
            data_frame_init = pd.read_csv(r"/home/fish991/code/Half_staggered/Env_wide/developed3.csv", encoding = "ISO-8859-1")
        elif episode ==2:
            data_frame_init = pd.read_csv(r"/home/fish991/code/Half_staggered/Env_wide/developed4.csv", encoding = "ISO-8859-1")
        elif episode ==3:
            data_frame_init = pd.read_csv(r"/home/fish991/code/Half_staggered/Env_wide/developed5.csv", encoding = "ISO-8859-1")
        elif episode ==4:
            data_frame_init = pd.read_csv(r"/home/fish991/code/Half_staggered/Env_wide/developed2.csv", encoding = "ISO-8859-1")
        elif episode ==5:
            data_frame_init = pd.read_csv(r"/home/fish991/code/Half_staggered/Env_wide/developed3.csv", encoding = "ISO-8859-1")
    ...
    #----------------------------------------------------------------------------------------------------------------------------------#
    def simulation(self, state_b, state_step, noise_actor, Noise,  T_state, episode, video_record_freq, snap_shot_freq, step_deadline, starting_act): 
        ...
        global dpdx_mean_gpu, dpdx_mean_init_gpu, past_dpdx_mean_gpu, CFL_num_gpu

        T_state_up = np.zeros([1], dtype=np.int32)
        bulk_v_return = np.zeros([1], dtype=np.float64)
        reward_interval = np.zeros([1], dtype=np.float64)
        T_state_end = T_state

        t_real = int(t_real)
        #if t_real == (step_deadline-1)*T_state: T_state = int(T_state + 1) #마지막부분 조정하는 부분이었다 t_real 499 -->500까지 이런
        #T_state = T_state + (t_real//((step_deadline)*T_state))
        
        #잠시 next_state_t memory활용해서 초기 state가져온다
        if t_real ==0:
            self.next_state_t[:,:,0] = self.state_stock[3]#self.next_state_t[:,:,0] = self.state_stock[1]; 
            self.state_interval_t = np.reshape(self.next_state_t, [1, self.state_shape[0]*self.state_shape[1]*self.state_shape[2]])

        for t in range(int(T_state)): #(t_real//step_deadline)간격이기때문에 1을 더 더해줘야 한다

            start = timer() 
            
            if t == 0:            
                action_output_b = noise_actor.predict(state_b) + Noise
                ...
                action_output_t = noise_actor.predict(-self.state_interval_t) + Noise
                ...
                action_output_b = noise_actor.predict(self.state_interval_b) + Noise#main_actor.predict(state) + Noise
                action_output_b = np.reshape(action_output_b, (1, nzp, nxp, self.action_channel)) 
                action_output_gpu_b = cp.asarray(action_output_b)
                action_output_gpu_b = cp.fft.rfft2(action_output_gpu_b, axes=(1,2))/(nxp*nzp)
                
                action_output_t = noise_actor.predict(-self.state_interval_t) + Noise#main_actor.predict(state) + Noise
                action_output_t = np.reshape(action_output_t, (1, nzp, nxp, self.action_channel)) 
                action_output_gpu_t = cp.asarray(action_output_t)
                action_output_gpu_t = cp.fft.rfft2(action_output_gpu_t, axes=(1,2))/(nxp*nzp)

            if t_real == 0: #(t_real은 진짜 시간이다)
                initial_coeff_gpu  =(friction_coff_b_gpu + friction_coff_t_gpu)/2 # for monitoring
                initial_coeff_reward_gpu = friction_coff_b_gpu # for reward            
    
            #up_gpu는 Fourier domain에 있다. (p --> Fourier domain) p가 없으면 real space
            if t == 0:
                past_dpdx_mean_gpu = dpdx_mean_gpu
                past_state_friction_gpu = (friction_coff_b_gpu + friction_coff_t_gpu)/2 # for monitoring
                past_friction_reward_gpu = friction_coff_b_gpu # for reward
            
            up_gpu[0:1] = 0.0; up_gpu[nyp-1:nyp] = 0.0; 
        
            #vp boundary condition # vp_bbc_gpu, vp_tbc_gpu
            if episode == 0 and t_real <= starting_act:
                vp_bbc_gpu[:] = action_output_gpu_b #-up_gpu[7:8,:,:,1:2]
                vp_tbc_gpu[:] = -action_output_gpu_t  #-up_gpu[nyp-1-7:nyp-7,:,:,1:2]
        
            else:
                vp_bbc_gpu[:] = action_output_gpu_b #-up_gpu[7:8,:,:,1:2] #-up_gpu[7:8,:,:,1:2]
                vp_tbc_gpu[:] = -action_output_gpu_t  #-up_gpu[nyp-1-7:nyp-7,:,:,1:2] #-up_gpu[nyp-1-7:nyp-7,:,:,1:2]
            ...
            dudy_gpu[1:nyp-1] = 0.5*(u_gpu[1:nyp-1]-u_gpu[0:nyp-2])/(dy_gpu[1:nyp-1]) \
                               +0.5*(u_gpu[2:nyp]-u_gpu[1:nyp-1])/(dy_gpu[2:nyp])
            #(u_gpu[2:nyp]-u_gpu[0:nyp-2])/(dy_gpu[1:nyp-1]+dy_gpu[2:nyp])
            ...
            #Get CFL_num
            CFL_num_gpu = CFL(up_storage_gpu, dx, dy_gpu, dz, dt, CFL_num_gpu)
            ...
            self.next_state_b[:,:,0] = next_state_stock[2] #self.next_state_b[:,:,0] = next_state_stock[0]; 
            self.next_state_t[:,:,0] = next_state_stock[3] #self.next_state_t[:,:,0] = next_state_stock[1]; 
            ...
            monitor.bulk_record(bulk_v_gpu, t_real, episode)
            
            #Drag reduction (%) recording every time_step each episode
            monitor.reduction_percentage(friction_coff_b_gpu, friction_coff_t_gpu, initial_coeff_gpu, t_real, state_step, episode)
            
            #Simulation information (drag coefficent, Operation speed, shear stress etc) every time step each episode
            monitor.simulation_info(t_real, dt, duration, mean_shear_b_gpu ,mean_shear_t_gpu, friction_coff_b_gpu, friction_coff_t_gpu, episode)

            #Check of Divergence free condition
            monitor.Divergence(t_real, div, episode)

            #Check of CFL num
            monitor.CFL_record(CFL_num_gpu, dt, t_real, episode)

            #Recording pressure gradient
            monitor.pressure_gradient(t_real, dpdx_mean_gpu, episode)
            #-------------------------------------------------------------------------------------#
            
            
            #----------------------------- take video periodically -------------------------------#
            if episode % video_record_freq == 0: #time_unit video
                monitor.time_video(up_gpu, self.next_state_b, vp_bbc_gpu, vp_tbc_gpu, t_real, episode, nxp, nyp, nzp, dx, dy, dz, y)
            #-------------------------------------------------------------------------------------#
            
            
            #----------------------------- take real_time snapshot -------------------------------#
            if t_real % snap_shot_freq == 0:
                monitor.snap_shot(up_gpu, self.next_state_b, vorticity_gpu, vp_bbc_gpu, vp_tbc_gpu, t_real, episode, nxp, nyp, nzp, dx, dy, dz, y)
            #-------------------------------------------------------------------------------------#
            
            #실제 time step에 대해 count해준다 (위에서 쓰이는 t는 실제시간이 아님)
            t_real += 1
            bulk_v_gpu = 0.0 #Initialization of bulk velocity (state에 대해서 뽑을거면 이거 자리를 좀 바꿔줘야함)
            
            if t_real == (step_deadline*T_state_end): # 예를 들어 t_real =1500까지 기록하고 싶으면 1501일때 0으로 바꿔줘야함. +1을 다시 빼줌
                t_real =0 #Episode 마지막에는 다시 t_real = 0으로 해줘야 한다
        ...
        ######################################### One state step simulation end (not time step) #########################################



        ##################################################### Reward function ###############################################
        
        #Drag coefficient reward
        initial_coeff = cp.asnumpy(initial_coeff_gpu); friction_coff_b = cp.asnumpy(friction_coff_b_gpu); friction_coff_t = cp.asnumpy(friction_coff_t_gpu) ;past_state_friction = cp.asnumpy(past_state_friction_gpu)
        initial_coeff_reward = cp.asnumpy(initial_coeff_reward_gpu); past_friction_reward = cp.asnumpy(past_friction_reward_gpu)
        #pressure reward
        dpdx_mean_init = cp.asnumpy(dpdx_mean_init_gpu); dpdx_mean = cp.asnumpy(dpdx_mean_gpu); past_dpdx_mean = cp.asnumpy(past_dpdx_mean_gpu)
            
        reduction_per_0 = ((friction_coff_b)/(initial_coeff_reward)) #((friction_coff_b+friction_coff_t)/(2*initial_coeff)) #current
        reduction_per_1 = (past_friction_reward/initial_coeff_reward) #just before state reduction

        reduction_pressure_0 = abs(dpdx_mean/dpdx_mean_init)
        reduction_pressure_1 = abs(past_dpdx_mean/dpdx_mean_init)

        long_term = reduction_pressure_0
        short_term = (reduction_pressure_0 - reduction_pressure_1)/(dt*T_state)

        # ---------------------------------------------- recording state unit ----------------------------------------------------#
        monitor.state_reduction_percentage(friction_coff_b_gpu, friction_coff_t_gpu, initial_coeff_gpu, initial_coeff_reward_gpu, state_step, t_real, T_state, episode)
        monitor.reward(-long_term, -short_term, -10*long_term, -0.2*short_term, state_step, t_real, T_state, episode) #record reward
        monitor.state_CFL_record(CFL_num_gpu, dt, state_step, t_real, T_state, episode)
        monitor.state_pressure_gradient(t_real, state_step, T_state, dpdx_mean_gpu, past_dpdx_mean_gpu, episode)
        # ------------------------------------------------------------------------------------------------------------------------#
        
        reward_drag = -0.0*(reduction_per_0) - 0.2*(reduction_per_0 - reduction_per_1)/(dt*T_state) 
        reward_pressure = -1.0*(long_term) - 0.0*(short_term) 

        reward = 0.0*reward_drag + 1.0*reward_pressure

        avg_reward = reward #summation/N
        
        
        print("avg_reward")
        print(avg_reward)

        if reduction_per_0 > 5.0 or reduction_per_0 < 0.5  or np.isnan(reduction_per_0) == True: 
            end = True
            t_real = 0
        ...
        return next_state_stock, reward, bulk_v_return, end
    #----------------------------------------------------------------------------------------------------------------------------------#



    def test_performance(self, main_actor, end_step, episode_t, video_record_freq_t, snap_shot_freq_t): 
        ...
        if t_real ==0: #top 부분 state받아오 주는거 <맨 위에 reset부분에서 self.state_stock 생성자로 해서 받아와준다# main에서 받아오는건 bottom만 데려온다
            ...
            self.next_state_b[:,:,0] = self.state_stock[2]#; self.next_state_b[:,:,1] = self.state_stock[2]
            self.state_interval_b = np.reshape(self.next_state_b, [1, self.state_shape[0]*self.state_shape[1]*self.state_shape[2]])            
            
            self.next_state_t[:,:,0] = self.state_stock[3] #; self.next_state_t[:,:,1] = self.state_stock[3]
            self.state_interval_t = np.reshape(self.next_state_t, [1, self.state_shape[0]*self.state_shape[1]*self.state_shape[2]])
        ...
        for t in range(int(end_step)): 
            ...
            #[nzp, nxp] 필요한 정보만 따로 빼내어 주도록 하자 
            next_state_stock = list(next_state_stock)
            self.next_state_b[:,:,1] = next_state_stock[2] #self.next_state_b[:,:,0] = next_state_stock[0]; 
            self.next_state_t[:,:,1] = next_state_stock[3] #self.next_state_t[:,:,0] = next_state_stock[1]; 
            ...
            #----------------------------- take video periodically -------------------------------#
            monitor.time_video(up_gpu, self.next_state_b, vp_bbc_gpu, vp_tbc_gpu, t_real, episode_t, nxp, nyp, nzp, dx, dy, dz, y)
            #-------------------------------------------------------------------------------------#
            
            #----------------------------- take real_time snapshot -------------------------------#
            if t_real % snap_shot_freq_t == 0:
                monitor.snap_shot(up_gpu, self.next_state_b, vorticity_gpu, vp_bbc_gpu, vp_tbc_gpu, t_real, episode_t, nxp, nyp, nzp, dx, dy, dz, y)
            #-------------------------------------------------------------------------------------#
