...
shear_action_b = np.zeros([nzp, nxp//2+1], dtype = np.float64)
shear_action_t = np.zeros([nzp, nxp//2+1], dtype = np.float64)
shear_cross_b = np.zeros([nzp, nxp], dtype = np.float64)
shear_cross_t = np.zeros([nzp, nxp], dtype = np.float64)

shear_action_b_gpu = cp.asarray(shear_action_b)
shear_action_t_gpu = cp.asarray(shear_action_t)
shear_cross_gpu = cp.asarray(shear_cross_b)
...
#---------------------- Cross val coefficient ------------------------#
def cross_val(shear):

    global nzp, nxp, shear_cross_gpu, cross_n

    shear = cp.asarray(shear)

    for i in range(nxp):
        #print(i)
        #print(shear_cross_gpu[:,0+i:i+1].shape)
        #print(shear[:, (corss_n+i)%nxp:1+(corss_n+i)%nxp].shape)
        shear_cross_gpu[:,0+i:i+1] = shear[:, (cross_n+i)%nxp:1+(cross_n+i)%nxp]

    return shear_cross_gpu

#------------------- Get Correlation coefficient ---------------------#
def correlation(shear, action):

    Cov_e = cp.mean((shear[:]- cp.mean(shear[:]))*(action[:]-cp.mean(action[:])))
    Corr_coef = Cov_e/(cp.std(shear[:])*cp.std(action[:]))
...
class env(): 
    ...
    def reset(self, episode): #초기화하고 state를 보내준다
        ...
        if episode == "Test":
            ...
            #data_frame_init = pd.read_csv(r"/home/fish991/code/Half_staggered/Test_folder/Double_Q/developed field (dudw)/mid dataset epTest t_real15000.csv", encoding = "ISO-8859-1")
        ...
        bulk_v_gpu = bulk_v_gpu/(float(yl)) #float(2800/180) # 
        ...
        #(dw/dy - wall)
        shear_wall_dwdy = (up_storage_gpu[1:2,:,:,0:1] - up_storage_gpu[0:1,:,:,0:1])/(Re*dy_gpu[1:2])
        ...
        t_real_temp = 0.0000001
        episode_t = "exact initial field"

        monitor.snap_shot(up_gpu, next_state_stock, shear_wall_dudy, shear_wall_dwdy, rms_vorticity_gpu, vorticity_gpu, vp_bbc_gpu, vp_tbc_gpu, t_real_temp, episode_t, nxp, nyp, nzp, dx, dy, dz, y)
        #------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def test_performance(self, main_actor, end_step, episode_t, video_record_freq_t, snap_shot_freq_t, scatter_freq): 
        ...
        global nxp, nyp, nzp, dy_gpu, dz, dx, alpha_gpu, gamma_gpu, t_real, Action_num, yl, wall, y
        global up_gpu, up_storage_gpu, up_mid_gpu, vp_bbc_gpu, vp_tbc_gpu
        global friction_coff_b_gpu , friction_coff_t_gpu, initial_coeff_gpu,  pp_gpu, dpdx_mean_init_gpu
        global H1p_gpu, H2p_gpu, dudx_gpu, dudy_gpu, dudz_gpu
        ...
        if t_real ==0: #top 부분 state받아오 주는거 <맨 위에 reset부분에서 self.state_stock 생성자로 해서 받아와준다# main에서 받아오는건 bottom만 데려온다
            ...
            self.state_interval_b = np.reshape(self.next_state_b, [1, self.state_shape[0]*self.state_shape[1]*self.state_shape[2]])    
            self.shear_b_analysis = self.state_stock[0]        
        ...
        for t in range(int(end_step)):
            ...
            #shear field
            state_field = cp.reshape(self.shear_b_analysis, [nzp,nxp])

            #Cross
            return_field = cross_val(state_field)

            #print(state_field.shape)
            ...
            # ----- Action contour ---- #
            action_field = cp.reshape(action_output_gpu_b, [nzp,nxp])
            #print(action_field.shape)
            # ------------------------- #

            action_output_gpu_b = cp.fft.rfft2(action_output_gpu_b, axes=(1,2))/(nxp*nzp)

            #Get correlation action field & Shear field
            Corre_action_shear = correlation(return_field, action_field)
        

            f_mean = open(Test_time_avg + '/action_shear_correlation.plt', 'a', encoding='utf-8', newline='')
            if t ==0: f_mean.write('VARIABLES = "t", "Correlation_shear_action" \n')	
            f_mean.write('%.5f %.5f \n' %(t, Corre_action_shear))    
            f_mean.close()
            ...
            #----------------------------------------------------- Opposition control -----------------------------------------------------#
            #vp_bbc_gpu[:] = 0.0 #-up_gpu[32:33,:,:,1:2]
            #vp_tbc_gpu[:] = 0.0 #-up_gpu[nyp-33:nyp-32,:,:,1:2]
            ...
            '''
            #Coherent Structure print
            if t%50 ==0 and t<24000:
                ...
                gradient_u = cp.concatenate((dudx_gpu[1:nyp-1], dudy_gpu[1:nyp-1], dudz_gpu[1:nyp-1]), axis=3)/Re

                gradient_u = cp.reshape(gradient_u, [nyp-2,nzp,nxp,3,3])
                gradient_uT = cp.transpose(gradient_u, [0,1,2,4,3])
                S_gpu = 0.5 *(gradient_u + gradient_uT)

                A_gpu = 0.5 *(gradient_u - gradient_uT)

                #3,4 axes로 Matmul하기
                R_1 = cp.einsum('bik,bkj->bij',S_gpu.reshape((nyp-2)*nxp*nzp,3,3), S_gpu.reshape((nyp-2)*nxp*nzp,3,3))
                R_2 = cp.einsum('bik,bkj->bij',A_gpu.reshape((nyp-2)*nxp*nzp,3,3), A_gpu.reshape((nyp-2)*nxp*nzp,3,3))

                Sum_gpu = R_1[:] + R_2[:]
    
                #Eigenvalue
                lam , _ = np.linalg.eigh(cp.asnumpy(Sum_gpu))
                lam = np.sort(lam)
                lam_2 = lam[:,1].reshape(nyp-2,nzp,nxp)

                if t<4000:
                    movie_index = int(1)
                elif t>=4000 and t<8000:
                    movie_index = int(2)
                elif t>=8000 and t<12000:
                    movie_index = int(3)
                elif t>=12000 and t<16000:
                    movie_index = int(4)
                elif t>=16000 and t<20000:
                    movie_index = int(5)
                elif t>=20000 and t<24000:
                    movie_index = int(6)


                f_CS = open(Test_Video_path + '/Coherent Structure movie_{}.plt'.format(movie_index), 'a')   
        
                if t==0 or t==4000 or t==8000 or t==12000 or t==16000 or t==20000: f_CS.write('VARIABLES="x","y","z","lam_2"\n') #t%4000==0
                f_CS.write('Zone T="HIT%d"\n'%0)
                f_CS.write('I=%d J=%d K=%d\n'%(nxp,(nyp-2)//2,nzp))

        
                for k in range(nzp):
                    for j in range((nyp-2)//2):
                        for i in range(nxp):    
                            f_CS.write('%.5f,%.5f,%.5f,%.5f \n' %(i*dx,y[j+1],k*dz,lam_2[j,k,i]))
            ...
            if t > starting_avg:
                ...
                if t > (fluc_nums+starting_avg) and t % time_avg_snap ==0:
                    ...
                    Two_correlation.write('VARIABLES = "x", "Correlation_f", "Correlation_g"\n')#, "Correlation_w"

                    for i in range(nxp):
                        Two_correlation.write('%.5f %.5f %.5f\n'%(i*dx, R_11_stream_gpu[i], R_22_stream_gpu[i]))#, R_33_stream_gpu[i]
            ...
            if t> starting_avg and t % time_avg_snap == 0:
                ...
                vorticity_t_avg = cp.asnumpy(vorticity_t_avg_gpu)
            ...
            self.shear_b_analysis = next_state_stock[0]
