...
wall = 9
#실제로는 +3 위치임
#10 위치 y+ ~ 1.6, 11 위치 y+ ~1.87 12 위치 y+ ~ 2.1 13위치 y+ ~2.5
...
class env(): 
    ...
    def simulation(self, state_b, state_step, noise_actor, Noise,  T_state, episode, video_record_freq, snap_shot_freq, step_deadline, starting_act): 
        ...
        for t in range(int(T_state)): #(t_real//step_deadline)간격이기때문에 1을 더 더해줘야 한다
            ...
            reward_drag = -10*(reduction_per_0) - 6.0*(reduction_per_0 - reduction_per_1)/(dt*T_state) 
            reward_pressure = -10*(reduction_pressure_0) - 0.5*(reduction_pressure_0 - reduction_pressure_1)/(dt*T_state) 

            reward = 0.0*reward_drag + 1.0*reward_pressure

            reward_interval = (reward + reward_interval)/T_state #summation/N
            '''
